...
case class Talk(titre: String, speaker: String, annee: Int, description: String, typeTalk: String)
...
val nbTalkParSpeaker = cc.sql("select annee, name, count(*) as nb from talks_par_speaker a JOIN devoxx.speakers b ON a.speaker = b.speaker group by annee, name")
nbTalkParSpeaker.registerTempTable("nb_talk_par_speaker")

val nbTalkParSociete = cc.sql("select annee, b.company, count(*) as nb  from talks_par_speaker a JOIN devoxx.speakers b ON a.speaker = b.speaker group by annee, b.company")
...
// Insertion des comptages par speaker et par annéee
cc.sql("insert into devoxx.speaker_par_annee select name, annee, nb from nb_talk_par_speaker").collect()
...
cc.sql("insert into devoxx.societe_par_annee select company, annee, nb from nb_talk_par_societe where company!='' and company is not null").collect()
